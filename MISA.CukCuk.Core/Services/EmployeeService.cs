    ...
    public class EmployeeService:BaseService<Employee>, IEmployeeService
    {
        ...
        protected override void ValidateCustom(Employee entity)
        {
            if (entity is Employee)
            {
                ...
                var isExists = _empRepository.CheckEmployeeCodeExists(employee.EmployeeCode);
                if (isExists == true)
                {
                ...
                if (_empRepository.CheckEmailExists(employee.Email))
                {
                    throw new GuardException<Customer>("Email đã tồn tại trong hệ thống, vui lòng kiểm tra lại", null);
                }

                if (_empRepository.checkIdentifyNumberExists(employee.IdentifyNumber))
                {
        ...
        {
            String NewCode = "";
            var Employeecode = _empRepository.GenEmployeeCode();
        ...
        {
            return _empRepository.SearchEmployees(name);
        }

        public bool CheckEmployeeCodeExist(string Code)
        {
            return _empRepository.CheckEmployeeCodeExists(Code);
        }

        public IEnumerable<Employee> SearchByDepartmentId(Guid department)
        {
            return _empRepository.SearchByDepartmentId(department);
        }
    }
}
